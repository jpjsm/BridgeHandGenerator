...
namespace BridgeHandGenerator.deal
{
    public class Deck
    {
        public const int DEFAULT_SHUFFLES = 16;
        ...
        private static uint shuffle_passes = DEFAULT_SHUFFLES;
        ...
        public void Reset(bool shuffle=false, uint shuffle_times = DEFAULT_SHUFFLES) 
        {
        ...
        public void Shuffle() 
        {
            for (uint shuffle_pass = 0; shuffle_pass < shuffle_passes; shuffle_pass++)
            {
                ...
                {
                    Cards card = deck[j];
                    int swap_with = RandomNumberGenerator.GetInt32(deck.Count);
        ...
        public Players_Cards Deal(bool shuffle=true)
        {
            ...
            return new Players_Cards(new Hand(hands[0]), new Hand(hands[1]), new Hand(hands[2]), new Hand(hands[3]));
        }

        public Players_Cards Deal(List<Hand_constraints> hand_constraints)
        {
